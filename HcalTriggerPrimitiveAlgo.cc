...
void HcalTriggerPrimitiveAlgo::analyzeQIE11(IntegerCaloSamples& samples,
  ...
  for (unsigned int ibin = 0; ibin < tpSamples; ++ibin) {
    ...
    //    if ((ids[0].ieta() == 1) && detId.iphi() == 1) {
    if ((ids[0].ieta() == 1) && ids[0].iphi() == 1) {
      //      std::cout << "idx = " << idx << " which is ibin+shift-filterPresamples, ibin = " << ibin << ", shift = " << shift << ", filterPresamples = " << filterPresamples << std::endl;
      //      std::cout << "dgPresamples = " << dgPresamples << ", dgSamples = " << dgSamples << ", tpPresamples " << tpPresamples << ", tpSamples = " << tpSamples << ", filterPresamples = " << filterPresamples << ", filterSamples = " << filterSamples << ", shift = " << shift << ", shrink = " << shrink << std::endl;
      for (int i = 0; i < 7; i++) {
        if ((timingTDC[idx+filterPresamples][i].first.second == 0 && timingTDC[idx+filterPresamples][i].second.first != 63 && timingTDC[idx+filterPresamples][i].second.first != 62 && timingTDC[idx+filterPresamples][i].second.first != 0) || (timingTDC[idx+filterPresamples][i].first.second == 1 && timingTDC[idx+filterPresamples][i].second.first != 3) ) {
	  std::cout << "in HcalTriggerPrimitiveAlgo.cc (just before TDC, linearized ADC, group0 passed to HcalFinegrainBit.cc) TDC = " << timingTDC[idx+filterPresamples][i].second.first << " with energy " << timingTDC[idx+filterPresamples][i].second.second << " at ieta=" << ids[0].ieta() << ", iphi=" << ids[0].iphi() << ", depth=" << i+1 << " and group0 bits = " << timingTDC[idx+filterPresamples][i].first.first << " for idx+filterPresamples (index of qie11df), ibin = " << idx+filterPresamples << ", " << ibin << " (where SOI is ibin=2)" <<  std::endl;
        }
      }
    }
    ...
    if ((ids[0].ieta() == 1) && ids[0].iphi() == 1) {
      std::cout << "finegrain[ibin] = " << finegrain[ibin] << " = computed quantities from TDC, ADC above, passed to compute function" << std::endl;
      std::cout << "(ICS) samples[ibin] " << samples[ibin] << " and (digi) result[ibin] = " << result[ibin] << std::endl;
    }
  }
  outcoder_->compress(output, finegrain, result);
}
...
void HcalTriggerPrimitiveAlgo::addUpgradeTDCFG(const HcalTrigTowerDetId& id, const QIE11DataFrame& frame) {
  ...
  if (detId.ieta() == 1 && detId.iphi() == 1) {
    std::cout << samples1.size() << " = size of samples1 and of frame nTS = " << frame.size() << ". The SOI for samples1 = " << samples1.presamples() << " and for QIEdataframe = " << frame.presamples() << " = frame.presamples() and frame.samples() = " << frame.samples() << std::endl;
    for (int i = 0; i < frame.samples(); i++) {
      std::cout << "for i = " << i << " frame[i].tdc() = " << frame[i].tdc() << " and frame[i].adc() = " << frame[i].adc() << " and samples1[i] = " << samples1[i] << " after linearization" << std::endl;
    }
  }
