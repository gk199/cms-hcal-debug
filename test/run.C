...
int main(int argc, char* argv[])
{
  ...
  std::map<int, TH2F*> frac_depth_exl_HB_2; // 10<et<=30
  std::map<int, TH2F*> frac_depth_exl_HB_3; //30<et
  for(int eta=0;eta<_length_ietaHB;eta++)
  {
    ...
    frac_depth_exl_HB_2[_ietaHB[eta]] = new TH2F(Form("Fraction_Depth_HB_Abs(eta)%d_2",_ietaHB[eta]), Form("TP Energy fraction HB vs Depth Abs(eta)%d 10<et<=30",_ietaHB[eta]),8,-0.5,7.5,60,0,1.2);
    frac_depth_exl_HB_3[_ietaHB[eta]] = new TH2F(Form("Fraction_Depth_HB_Abs(eta)%d_3",_ietaHB[eta]), Form("TP Energy fraction HB vs Depth Abs(eta)%d 30<et",_ietaHB[eta]),8,-0.5,7.5,60,0,1.2);
   }
  ...
  std::map<int, TH2F*> frac_depth_exl_HE_2; // 10<et<=30        
  std::map<int, TH2F*> frac_depth_exl_HE_3; //30<et     
  for(int eta=0;eta<_length_ietaHE;eta++)
    {
      ...
      frac_depth_exl_HE_2[_ietaHE[eta]] = new TH2F(Form("Fraction_Depth_HE_Abs(eta)%d_2",_ietaHE[eta]), Form("TP Energy fraction HE vs Depth Abs(eta)%d 10<et<=30",_ietaHE[eta]),8,-0.5,7.5,60,0,1.2);
      frac_depth_exl_HE_3[_ietaHE[eta]] = new TH2F(Form("Fraction_Depth_HE_Abs(eta)%d_3",_ietaHE[eta]), Form("TP Energy fraction HE vs Depth Abs(eta)%d 30<et",_ietaHE[eta]),8,-0.5,7.5,60,0,1.2);
    }
	  ...
	  if (abs(ieta) <= 15 )
	    {
              ...
              if (et <= 10 )                 frac_depth_exl_HB_1[abs(ieta)]->Fill(i, TP_energy_depth[i]/et);
              else if (et <=30 && et > 10 )  frac_depth_exl_HB_2[abs(ieta)]->Fill(i, TP_energy_depth[i]/et);
              else if (et > 30 )             frac_depth_exl_HB_3[abs(ieta)]->Fill(i, TP_energy_depth[i]/et);
	    }
	    ...
	    {          
	      ...
	      if (et <= 10 )                 frac_depth_exl_HE_1[abs(ieta)]->Fill(i, TP_energy_depth[i]/et);
	      else if (et <=30 && et > 10 )  frac_depth_exl_HE_2[abs(ieta)]->Fill(i, TP_energy_depth[i]/et);
	      else if (et > 30 )             frac_depth_exl_HE_3[abs(ieta)]->Fill(i, TP_energy_depth[i]/et);
	    }
	}
    }
  }
  ...
  for(int eta=0;eta<_length_ietaHE;eta++)
  {
    ...
    frac_depth_exl_HE_3[_ietaHE[eta]]->Write();
  }
  ...
  for(int eta=0;eta<_length_ietaHB;eta++)
    {
      ...
      frac_depth_exl_HB_3[_ietaHB[eta]]->Write();
    }
