  ...
  //  triggerBits_( consumes<edm::TriggerResults>(config.getParameter<edm::InputTag>("bits")) )
{
   ...
   SOIenergy_vs_event_ = fs->make<TH2D>("SOIenergy_vs_event","Energy in SOI vs event number",100,0,10000,256,0,255);
...
//   setup.get<CaloGeometryRecord>().get(tpd_geo);
   ...
   */
   ...
   for (const auto& digi: *digis) {
	...
	SOIenergy_vs_event_->Fill(event.id().event(),digi.SOI_compressedEt());
